    ...
    <div class="deviceCreateContainer">
        ...
        <FormLayout v-if="state.loaded" :on-submit="handleSubmit" :data="state" method="POST">
            ...
            <select name="inputSelectCompany" id="inputSelectCompany" v-model="state.idCompany">
                <option v-for="(item) of state.companies" :id="`company-${item.id}`" :value="item.id">
                    {{ item.name }}
                </option>
            </select>
            ...
            <input type="submit" class="btn btn-warning my-2" value="Atualizar">
        </FormLayout>
        <div v-else class="mx-auto p-1 text-center fs-4">
            <p class="loadingTextStyle">Aguarde...</p>
        </div>
        <div class="mx-auto p-1 text-danger text-center fs-5">
...
const state = reactive({
    id: props.device.id,
    idCompany: props.device.idCompany,
    plate: props.device.plate,
...
const handleSubmit = async () => {
    try{
        ...
        const response = await fetcher.usePutRequest(CONSTANTS.URL_DEVICE + '/' + state.id, newDevice);
